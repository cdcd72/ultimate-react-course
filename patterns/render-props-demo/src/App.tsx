...
const products: IProduct[] = Array.from({ length: 20 }, () => {
  return {
...
const companies: ICompany[] = Array.from({ length: 15 }, () => {
  return {
  ...
  return (
    <div>
      ...
      <div className="col-2">
        ...
        <ProductListWithToggles title="ProductList HOC" items={products} />
      </div>
    </div>
  );
}

function ProductListWithToggles({
  title,
  items,
}: {
  title?: string;
  ...
  return withToggles(ProductList)({ title, items });
}
